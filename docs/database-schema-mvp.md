...
| case_diameter_mm | decimal(4,1), nullable | діаметр корпусу в мм, з дробовою частиною (напр. 38.5); фільтр діапазонами (напр. 38-41) |
| case_thickness_mm | decimal(4,1), nullable | товщина корпусу в мм, з дробовою частиною (напр. 9.2); окреме число (не EAV) за тим самим принципом, що діаметр — фільтр діапазоном (min/max), не список значень |
...
Восьма звірка (тестовий імпорт Casio, реальні дані виробника):
23. `case_diameter_mm` і `case_thickness_mm` — з `int` на `decimal(4,1)`. Реальні технічні дані виробників часто дробові (38.5мм, 44.2мм — стандартний спосіб зазначення розміру годинника, не помилка й не неточність), ціле число їх обрізало. `decimal`, а не `float`/`double precision` — той самий підхід, що вже в `products.price`: точне десяткове представлення без похибок бінарного float, і саме воно порівнюється в діапазонних фільтрах (`>=`/`<=`), де похибка float небажана.
